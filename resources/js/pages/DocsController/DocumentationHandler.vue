<template>
    <div class="min-h-screen bg-gray-100 flex items-start">
        <div class="w-64 flex-shrink-0 sticky top-0 text-right border-r-2 border-pink-550 min-h-screen shadow-2xl mr-8 bg-blue-500 text-dark text-lg py-4">
            <RouterLink :to="{ name: 'DocsController@LandingPage' }" v-slot="{ href, navigate }">
                <a :href="href" @click="navigate" class="font-display text-6xl cursor-pointer transform -skew-y-12 text-shadow-lg mx-4 py-8">Phase</a>
            </RouterLink>
            ...
            <ul class="mt-8">
                <RouterLink
                    ...
                    :key="section.id"
                >
                    <li class="p-4 flex flex justify-end cursor-pointer hover:bg-blue-600" @click="navigate">
                        <a :class="isExactActive ? 'border-pink-550': 'border-transparent'" class="w-full border-b-2" :href="href">{{ section.title }}</a>
                    </li>
                </RouterLink>
